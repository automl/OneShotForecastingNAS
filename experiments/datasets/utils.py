import collections
from typing import Mapping,  Sequence, Union, Iterator
...
# the codes here are adapted from https://github.com/automl/Auto-PyTorch/blob/master/autoPyTorch/pipeline/components/training/data_loader/time_series_util.py
...
def pad_sequence_with_minimal_length(sequences: list[torch.Tensor],
                                     ...
                                     sample_interval: int=1) -> torch.Tensor:
    r"""
    ...
    if sample_interval > 1:
        first_indices = -(sample_interval * ((max_len - 1) // sample_interval) + 1)
        sample_indices = torch.arange(first_indices, 0, step=sample_interval)
        max_len = len(sample_indices)
    ...
    if sample_interval > 1:
        for i, tensor in enumerate(sequences):
            sample_indices_tensor = sample_indices[sample_indices >= -tensor.size(0)]
            tensor = tensor[sample_indices_tensor]
...
class PadSequenceCollector:
    """
    A modification version of torch.utils-data_utils_collate
    ...
    We consider two types of sample interval.
    The first one is sample_interval. This is introduced by
    the 'resolution' fidelity type and results in a shorter sequence. The other one is sample_interval_fix_seq_len,
    the sequence's length is not changed after the sub-sampling. This allows the model to create long-term dependencies
    and is introduced by the dataloader's hyperparameter. Therefore, a sample_interval_fix_seq_len > 1 indicates that
    our sampler needs to pre-select a sequence with a larger length
    """

    def __init__(self,
                 window_size: int,
                 sample_interval: int = 1,
                 target_padding_value: float = 0.0,
                 seq_max_length: int = np.iinfo(np.int32).max):
...
class TimeSeriesSampler(SubsetRandomSampler):
    ...
    def __init__(self,
                 indices: Sequence[int],
                 seq_lengths: Union[Sequence[int], np.ndarray],
                 num_instances_per_seqs: Union[list[float], np.ndarray] | None = None,
                 min_start: int = 0,
